...
public class ControllerTareas extends Functions{
	...
	public void loadCbxCategory(JComboBox<Categoria> cbx) {
		 ...
		 * Tambien guarda la seleccion para que si se tenia seleciconada una categoria
	...
	public Object [][] getTaskData(Categoria category){ 
		...
		for(Tarea item : listaTareas.getAll()) {
			if(item.getCategoryId()==category.getId() && !item.getCurrentState()) {
				tareas.add(item);
			}
		}
	...
	public void initTareasPendientes() {
	    ...
	    vPendientes.getBtnCompletada().addActionListener(e -> {
	        ...
	        if (id > 0) {
	            int opcion = JOptionPane.showConfirmDialog(
	                null,
	                "Desea marcar como completada la tarea?"
	            );
	...
	/*Este metodo esta apartado para utilizarlo en el boton cargar y actualizar la tabla en otros
	 * Sin tener que reiniciar todo el panel entero (la tabla arranca vacia si no)
	 */
	...
	public void editTask(int id) {
		...
		loadCbxCategory(v.getCbxCategory());
		v.getCbxCategory().setSelectedIndex(item.getCategoryId()-1);
